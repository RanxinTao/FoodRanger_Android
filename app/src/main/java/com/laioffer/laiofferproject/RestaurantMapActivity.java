...
public class RestaurantMapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // This function will call onMapReady()
        mapFragment.getMapAsync(this);

        Bundle bundle = this.getIntent().getExtras(); // getExtras: retrieve bundle
        if (bundle != null) {
    ...
    @Override
    // implements OnMapReadyCallback and Google map will call this function to show a marker
    public void onMapReady(GoogleMap map) {
        if (toMark != null) {
            map.addMarker(new MarkerOptions().position(toMark).title("Marker"));
            map.moveCamera(CameraUpdateFactory.newLatLng(toMark));
            map.animateCamera(CameraUpdateFactory.zoomTo(11), 2000, null); // zoom in 11x in 2000ms
        }
    }
